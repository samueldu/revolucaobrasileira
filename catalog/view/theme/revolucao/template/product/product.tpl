   	...
   	<div style="width: 100%; margin-bottom: 10px;">
        ...
        <div style="width:60%; float:right;">
        	...
        	<tr>
        	 ...
        	 <div class="modelCinza"><?php echo $model; ?></div>
        	 </td>
              </tr>
              ...
              <tr>
                ...
                <td class="linePrice"> 
                <?php if (!$special) { ?></b>
                <b><?php echo $price; ?></b><span class="apenas"> a vista ou</span>
              
              <?   
              $priceValue = str_replace("R$ ","",$price);
              for($i = 2; $i<=7; $i++){
			  $valorParcela = ($priceValue/$i);
			  if($valorParcela >= "1")
				$parcela[$i] = $valorParcela;
                 ...
                 <div class="parcelas">Em at&eacute; <span><?=count($parcela)?>x de R$ <?=number_format($priceValue/count($parcela), 2, ',', ' ');?>x</span> sem juros</div>
                  <?php } else { ?>
                  <span style="text-decoration: line-through;font-size:12px;color:#999"> de: <?php echo $price; ?></span> &nbsp; por: <b><?php echo $special; ?>
                  <?php } ?></td>
                  <td style="text-align:right">
                  <img src="catalog/view/theme/<?=TEMPLATE?>/image/frete-gratis.png" alt="Frete Gratis Armazem" /></td>
                  
                   <?php if ($review_status) { ?>
			 
                <td align="right" style="font-size:11px;color:#333" valign="top">
                <?php echo $text_average; ?> </td>
                ...
                &nbsp; <?php if ($average) { ?>
                  ...
                  <?php } else { ?>
                   <a onclick="javascript:window.location.hash='tab_review'"  style="font-size:11px;color:#888888"><?php echo $text_no_rating; ?></a>
                  <?php } ?></td>
            ...
            <form action="<?php echo str_replace('&', '&amp;', $action); ?>" method="post" enctype="multipart/form-data" id="product">
              ...
              <?php if ($options) {               
              if(isset($products_outras_cores))
              {
              ?>
              
              <div  class="content">
              <div>
                  <?php foreach ($products_outras_cores as $option) { 
                  ?>
                  <a href=<?=$option['href']?>><img src=<?=$option['thumb']?>></a>                  
              	  <?php } ?>
          ...
          ?>
                ...
                ?>
                Seu email: <input type="text" name="email" id="email" style="background:##FFCC33;">
                 ...
                 $variacoes = array();
					...
					$display = "none";
     				...
     				?>
               		...
               		<div class="content" id ="<?=$variations['id']?>" style="display:<?=$display?>; float:left; width:558px">
		              
		               <?
		                if($variations['quantity'] > 0)
		                {
			               ?>
			               <div class="variacao01">			               Quantidade: 
			               ...
			               <?       
				            ...
				            <div class="variacao02">
				            <a onclick="compra('<?=$variations['id']?>')"; id="add_to_cartx" class="button"><span><?php echo $button_add_to_cart; ?></span></a></div><div class="variacao03"><span>Adicionar produto &aacute; minha</span><br />
			            ...
			            {
			            ?>
				            Seu email: <input type="text" name="email" id="email<?=$variations['id']?>" style=" background:#faffbd">
...
if($config_face_comm_app){
	...
	<script>
	function openFace(){
		window.open("https://www.facebook.com/dialog/oauth?client_id=<?=$facebookCode?>&redirect_uri=http://www.modaarmazem.com.br/loja/account/facebook/&scope=email,read_stream,status_update,publish_stream","mywindow","menubar=0,resizable=0,width=1000,height=600");
	}
	</script>
	<?
	...
	{
?>	
<span class="shareFB"><strong>Compartilhe com seus amigos:</strong></span>
		      <div id="fb-root"></div>
		...
		<script src="catalog/view/javascript/facebook/TextboxList.Autocomplete.js" type="text/javascript" charset="utf-8"></script>
			...
			$(function(){
				...
				var t5 = new $.TextboxList('#form_tags_input_4', {unique: true, plugins: {autocomplete: {
					minLength: 2,
					queryRemote: true,
					remote: {url: 'http://www.modaarmazem.com.br/loja/account/autocomplete/'}
				}}});
...
<img src="catalog/view/theme/<?=TEMPLATE?>/image/share_face.gif" onclick="javascript:compartilhaFace();" /></div>
<?
}
?>	-->
...
?>
<span class="descriType">Comentar produto</span>
<div id="fb-root"></div>
<fb:comments href="<?=$location?>" num_posts="3" width="558" allowTransparency="true" frameborder="0" scrolling="no" style="float: right;margin: 5px 0 0;">
</fb:comments>		
<?
}
?>
    ...
    <div id="tab_review" class="tab_page">
      ...
      <!--div class="content round"><b><?php echo $entry_name; ?></b><br /-->
        <input type="hidden" name="namex" id="namex" value="Voto do site" />
        <input type="hidden" name="text" id="text" value="Voto do site." />
        <!--br />
        ...
        <b><?php echo $entry_captcha; ?></b><br />
         ...
         <table>
         	<tr>
            	<td><img src="index.php?route=product/product/captcha" id="captcha" /></td>
                
            <td style="padding-bottom:10px"><input type="text" name="captcha" value="" autocomplete="off" /><a onclick="review();" class="button"><span><?php echo $button_comment; ?></span></a></td>
            </tr>
         </table>
...
function review() {
	$.ajax({
		...
		dataType: 'json',
		data: 'name=' + encodeURIComponent($("#namex").val()) + '&text=' + encodeURIComponent($("#text").val()) + '&rating=' + encodeURIComponent($('input[name=\'rating\']:checked').val() ? $('input[name=\'rating\']:checked').val() : '') + '&captcha=' + encodeURIComponent($('input[name=\'captcha\']').val()),
		beforeSend: function() {
