<?php 
class ModelPaymentiPagareWeb extends Model {
  	public function getMethod($address) {
		$this->load->language('payment/ipagare');
		
		if ($this->config->get('ipagare_status')) {
			...
			if (!$this->config->get('ipagare_geo_zone_id')) {
        		$status = TRUE;
		...
		if ($status) {  
      		$method_data = array( 
				...
				'sort_order' => $this->config->get('ipagare_sort_order')
      		);
    	}
