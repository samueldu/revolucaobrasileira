...
class ControllerProductProduct extends Controller {
	...
	public function index() { 
		...
		$this->request->get['manufacturer_id'] = $product_info['manufacturer_id'];
		
		if (isset($this->request->get['manufacturer_id']) and $this->request->get['manufacturer_id'] !=0) {
			...
			if($manufacturer_info['name'] == "")
			$this->document->title = $this->config->get('config_name').", ".$product_info['name'];  
			else
			$this->document->title = $this->config->get('config_name').", ".html_entity_decode($product_info['name'],ENT_QUOTES,'UTF-8').", ".$manufacturer_info['name'];			
			...
			### My Wish List - Start ###
			$this->data['link_mywishlist'] = HTTP_SERVER . 'index.php?route=account/mywishlist&action=add&id='.$this->request->get['product_id'];
		    $this->data['button_add_to_mywishlist'] = $this->language->get('button_add_to_mywishlist');
		    ### My Wish List - End ###
			
			$this->data['text_rate_too'] = $this->language->get('text_rate_too');            
			...
			if ($this->config->get('config_face_comm')) {
				$this->data['config_face_comm_app'] = $this->config->get('config_face_comm_app');
			} else {
				$this->data['config_face_comm_app']  = false;
			}
			...
			$this->data['thumb_tiny']  = $this->model_tool_image->resize($image, 64, 48);
			...
			$this->data['estoqueReal'] = $product_info['quantity'];
			...
			if(isset($manufacturer_info['name']))
			$this->data['manufacturer_logo'] = $this->model_tool_image->resize($product_info['manufacturer_logo'], 50,40);
			...
			foreach ($results as $result) {
				$this->data['images'][] = array(
					...
					'thumb_tiny'    => $this->model_tool_image->resize($result['image'], 64,48)
				);
			}
			...
			$this->data['facebookCode'] = $this->config->get('config_face_comm_app');
				...
				if ($result['tag']) {
					$this->data['tags'][] = array(
						...
						'href'	=> HTTP_SERVER . 'index.php?route=product/search&keyword=' . $result['tag']
							...
							/* modificação mostrar produtos de outras cores para armazem */
				if(TEMPLATE == "armazem")
				{
					$product_info_search['model'] = substr($product_info['model'],0,3);
					
					if(substr_count($product_info['name'],"OFF WHITE"))
					{
						$product_info_search['name'] = substr($product_info['name'],0,strrpos($product_info['name'], " "));
						$product_info_search['name'] = substr($product_info_search['name'],0,strrpos($product_info_search['name'], " "));
					}	
					else
					{				
						$product_info_search['name'] = substr($product_info['name'],0,strrpos($product_info['name'], " "));
					...
					$category_info['category_id'] = FALSE;
					
					$optionsCores = $this->model_catalog_product->getProductsByKeyword($product_info_search['name'],$category_info['category_id'],FALSE,$product_info_search['model'],"p.sort_order","ASC",0,5,"AND");
					
					//print_r($optionsCores);
			
					if ($optionsCores) {
						
						foreach($optionsCores as $key=>$value){
						
						if($optionsCores[$key]['image']){	
							$image = $optionsCores[$key]['image'];
						} else {
							$image = 'no_image.jpg';
						}
			
						$this->data['products_outras_cores'][] = array(
							'product_id'    => $optionsCores[$key]['product_id'],
							'name'    		=> $optionsCores[$key]['name'],
							'model'   		=> $optionsCores[$key]['model'],
							'image'   		=> $this->model_tool_image->resize($image, 38, 38),
							'thumb'   		=> $this->model_tool_image->resize($image, $this->config->get('config_image_related_width'), $this->config->get('config_image_related_height')),
							'href'    		=> $this->model_tool_seo_url->rewrite(HTTP_SERVER . 'index.php?route=product/product&path=' . $path.'&product_id=' . $optionsCores[$key]['product_id']),
						);
...
*/
	...
	public function avise(){
		
		if(!isset($this->request->post['variacao']))
		$this->request->post['variacao'] = '';
		
		$this->load->model('catalog/product');
	    $this->model_catalog_product->avise($this->request->post['product_id'],$this->request->post['variacao'], $this->request->post['email']);
	}
}
?>
