	...
	public function index() { 
		...
		if ($manufacturer_info) {
			...
			if ($product_total) {
				...
				if($manufacturer_info['status_hotsite'] == 1){
					if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/hotsite/'.$manufacturer_info['template'].'.tpl')) {
						$this->template = $this->config->get('config_template') . '/template/hotsite/'.$manufacturer_info['template'].'.tpl';
					} else {
						$this->template = 'default/template/hotsite/'.$manufacturer_info['template'].'.tpl';
					}
				}else{
