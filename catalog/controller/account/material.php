<?php 
class ControllerAccountMaterial extends Controller {
	private $error = array();
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
      	...
      	$this->document->breadcrumbs[] = array(
        	'href'      => HTTP_SERVER . 'index.php?route=account/material',
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/material.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/account/material.tpl';
		} else {
			$this->template = 'default/template/account/material.tpl';
		}
		
        $this->load->model('checkout/extension');

        $module_data = $this->model_checkout_extension->getExtensionsByPosition('module', 'login');
