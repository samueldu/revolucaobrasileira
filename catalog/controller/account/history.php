	...
	public function index() {
		...
		if ($order_total) {
			...
			foreach ($results as $result) {
        		...
        		if($result['payment_method'] == "Ipagare" or $result['payment_method'] == "Ipagare web")
        		{
        			$resultx['payment_method'] = $this->model_account_order->getIpagare($result['order_id']);   
        			
        			if(@count($resultx['payment_method']) > 0)
        			$result['payment_method'] = $resultx['payment_method'][0]['meioF']." ".$resultx['payment_method'][0]['formaF'];
        		}
