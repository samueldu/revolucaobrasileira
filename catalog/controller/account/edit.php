...
class ControllerAccountEdit extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$data = $this->request->post['dataNasc'];
  			$this->request->post['dataNasc'] = implode(preg_match("~\/~", $data) == 0 ? "/" : "-", array_reverse(explode(preg_match("~\/~", $data) == 0 ? "-" : "/", $data)));
		...
		if (isset($this->request->post['cpfCnpj'])) {
			...
			$this->data['cpfCnpj'] = $customer_info['cpfCnpj'];
		} else {
			$this->data['cpfCnpj'] = '';
		}
