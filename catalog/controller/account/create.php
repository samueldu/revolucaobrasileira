...
class ControllerAccountCreate extends Controller {
	...
	public function index() {
		...
		if(isset($this->request->post['tipoPessoa']))
		$tipoPessoa = $this->request->post['tipoPessoa'];		
		else
		$tipoPessoa = "PF";
		...
		$this->data['tipoPessoa'] = $tipoPessoa; 
		
		$this->language->load('account/create');
		
		$this->language->load('account/create'.$tipoPessoa);
		...
		$this->data['entry_bairro'] = $this->language->get('entry_bairro');
		...
		$this->data['entry_dataNasc'] = $this->language->get('entry_dataNasc');
		...
		if (isset($this->error['dataNasc'])) {
			$this->data['error_dataNasc'] = $this->error['dataNasc'];
		} else {
			$this->data['error_dataNasc'] = '';
		}
		...
		if (isset($this->error['cpfCnpj'])) {
			$this->data['error_cpfCnpj'] = $this->error['cpfCnpj'];
		} else {
			$this->data['error_cpfCnpj'] = '';
		}	
		...
		if (isset($this->error['bairro'])) {
			$this->data['error_bairro'] = $this->error['bairro'];
		} else {
			$this->data['error_bairro'] = '';
		}
		...
		if (isset($this->request->post['dataNasc'])) {
			$this->data['dataNasc'] =  $this->request->post['dataNasc'];
		} else {
			$this->data['dataNasc'] = '';
		}
		...
		if (isset($this->request->post['cpfCnpj'])) {
			$this->data['cpfCnpj'] = $this->request->post['cpfCnpj'];
		} else {
			$this->data['cpfCnpj'] = '';
		}
		...
		if (isset($this->request->post['bairro'])) {
			$this->data['bairro'] = $this->request->post['bairro'];
		} else {
			$this->data['bairro'] = '';
		}
	...
	private function validate() {
		...
		if(((strlen($this->request->post['cpfCnpj']) < 15) and ($this->validateCpf($this->request->post['cpfCnpj'] === false))) or (strlen($this->request->post['cpfCnpj']) == 0)) {
			$this->error['cpfCnpj'] = $this->language->get('error_cpfCnpj');	
		}
		
		if(strlen($this->request->post['dataNasc']) == 0 and $tipoPessoa == "PF"){
			$this->error['dataNasc'] = $this->language->get('error_dataNasc');	
		}
		
		if(strlen($this->request->post['bairro']) == 0){
			$this->error['bairro'] = $this->language->get('error_bairro');	
		}
		...
		if(strlen($this->request->post['rg']) < 8){
		...
		if ((strlen(utf8_decode($this->request->post['address_3'])) < 1) || (strlen(utf8_decode($this->request->post['address_3'])) > 128)) {
	...
	public function postcode() {
		...
		$this->language->load('account/createPF');
	...
	public function postcep() {

		$this->language->load('account/createPF');

		$this->load->model('localisation/country');
				
		$cep_id = str_replace("-", "", $this->request->get['postcode']); 
		
		$result = $this->model_localisation_country->getCep($cep_id);
		
		foreach ($result as $key => $value){
			if($key !== 'estado' && $key !== 'cep')
			{
			
			$result[$key] =  ucwords($this->strtolower2($value));
			}
		}
	...
	public function trocaAcento( $string )
	{
		//$search = array( '�', '', 'Ã', 'Â', 'É', 'Ê', '�?', 'Ó', 'Õ', 'Ú', 'Ç' );
		//$replace = array( '�', 'à', 'ã', 'â', 'é', 'ê', 'í', 'ó', 'õ', 'ú', 'ç' );
	...
	public function strtolower2($texto) {
  ...
  return utf8_encode($palavra);
    ...
    public function addUser()
    {
        ...
        if ($this->model_account_customer->getTotalCustomersByEmail($this->request->post['email'])) {
            $result['warning'] = $this->language->get('error_exists');
        }
        else
        {
        ...
        */
        ...
        $result['url'] = HTTPS_SERVER . 'account/success';
