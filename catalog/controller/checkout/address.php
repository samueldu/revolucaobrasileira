...
class ControllerCheckoutAddress extends Controller {
	...
	public function shipping() {
  	...
  	private function getForm($type) {
        ...
        $this->data['button_use_this_address'] = $this->language->get('button_use_this_address');   
		...
		foreach ($results as $result) {
      		$this->data['addresses'][] = array(
	    		...
	    		'address'    => '<b>'.$result['company'].'</b> '.$result['firstname'] . ' ' . $result['lastname'] . ', ' . $result['address_1'] . ', ' . $result['city'] . ', ' . (($result['zone']) ? $result['zone']  . ', ' : FALSE) . (($result['postcode']) ? $result['postcode']  . ', ' : FALSE) . $result['country'],
        		'href'       => HTTPS_SERVER . 'index.php?route=account/address/' . $type . '&address_id=' . $result['address_id'],
        		'edit'       => HTTPS_SERVER . 'index.php?route=account/address/update&address_id=' . $result['address_id'].'&flag=1'
      		);
    	}
  	...
  	private function validate() {
  	 ...
  	 if ((strlen(utf8_decode($this->request->post['bairro'])) < 1) || (strlen(utf8_decode($this->request->post['bairro'])) > 128)) {
