...
class ControllerCheckoutConfirm extends Controller { 
    ...
    $this->session->data['confirm'] = "ok";
    
    //print $this->request->post['shipping_method']; 
    
    if(isset($this->request->post['shipping_method']))
    {
        $shipping = explode('.', $this->request->post['shipping_method']);
        
        if(!isset($this->session->data['shipping_method']) or $this->request->post['shipping_method']['id'] != $this->session->data['shipping_methods'][$shipping[0]]['quote'][$shipping[1]])
        {
    ...
    $this->session->data['formaPagamento'] = $this->request->post['formaPagamento'];   
    
    $idForma = $this->session->data['formaPagamento'];
    
    if(isset($this->request->post['tipo'.$idForma]))
    $this->session->data['tipo'.$idForma] = $this->request->post['tipo'.$idForma];
    
    if(isset($this->request->post['numberCard'.$idForma]))
    $this->session->data['numberCard'.$idForma] = $this->request->post['numberCard'.$idForma];
    
    if(isset($this->request->post['parcel'.$idForma]))
    $this->session->data['parcel'.$idForma] = $this->request->post['parcel'.$idForma];
    
    if(isset($this->request->post['yearCard'.$idForma]))
    $this->session->data['yearCard'.$idForma] = $this->request->post['yearCard'.$idForma];
    
    if(isset($this->request->post['monthCard'.$idForma]))
    $this->session->data['monthCard'.$idForma] = $this->request->post['monthCard'.$idForma];
    
    if(isset($this->request->post['securityCode'.$idForma]))
    $this->session->data['securityCode'.$idForma] = $this->request->post['securityCode'.$idForma];
    
    if(isset($this->request->post['nome'.$idForma]))
    $this->session->data['nome'.$idForma] = $this->request->post['nome'.$idForma];

        
    
    $this->session->data['payment_id'] = $idForma;  
    $this->data['payment_id'] = $idForma; 
    ...
    {    
        $this->session->data['payment_address_id'] = $this->session->data['shipping_address_id']['id'];            
        $this->session->data['payment_method']['id'] = "ipagare";
        $this->session->data['payment_method']['title'] = "Ipagare";
        $this->session->data['payment_method']['sort_order'] = "1"; 
    }
    else
    {
        $this->session->data['payment_address_id'] = $this->session->data['shipping_address_id']['id'];            
        $this->session->data['payment_method']['id'] = "ipagare_web";
        $this->session->data['payment_method']['title'] = "Ipagare web";
        $this->session->data['payment_method']['sort_order'] = "1"; 
    }   
    
    $this->data['idPagamento'] = $this->session->data['payment_method']['id'];
    
       
    if(($this->session->data['tipo'.$idForma]) == "cartao")
    {
        $this->session->data['payment_numberCard'] = $this->session->data['numberCard'.$idForma];
        $this->session->data['payment_parcel'] = $this->session->data['parcel'.$idForma];
        $this->session->data['payment_nameCard'] = $this->session->data['nome'.$idForma]; 
        $this->session->data['payment_yearCard'] = $this->session->data['yearCard'.$idForma];      
        $this->session->data['payment_monthCard'] = $this->session->data['monthCard'.$idForma];  
        $this->session->data['payment_securityCode'] = $this->session->data['securityCode'.$idForma];    
    }
    ...
    if($this->session->data['coupon'] != "" and $this->request->server['REQUEST_METHOD'] == 'POST')
    { 
   
           if($this->validateCoupon()) {
                              
            $this->session->data['coupon'] = $this->session->data['coupon'];
        ...
        if ($this->cart->hasShipping()) {
            ...
            if (!isset($this->session->data['shipping_method'])) {   
                ...
                $this->session->data['erro'] = "error_shipping";
        ...
        $data['cpfCnpj'] = $this->customer->getCpfCnpj();        
        $data['dataNasc'] = $this->customer->getDataNasc();        
        ...
        if ($this->cart->hasShipping()) {
            ...
            $data['shipping_bairro'] = $shipping_address['bairro'];
        ...
        $data['payment_bairro'] = $payment_address['bairro'];
   ...
   /*       $this->document->breadcrumbs[] = array(
        ...
        if ($payment_address) {
            ...
            $replace = array(
                ...
                'address_3' => $payment_address['address_3'],
                'bairro'    => $payment_address['bairro'],
    ...
    private function validateCoupon() {
        ...
        if (!$coupon) {
            $this->session->data['warning'] = $this->language->get('error_coupon');
            $this->redirect(HTTPS_SERVER . 'index.php?route=checkout/shipping');  
        } 
