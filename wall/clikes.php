...
if(@$action == 1)
{
	$result = mysql_query("update wallcomments set clikes=clikes+1 where c_id= ".$entry_id);
	$result = mysql_query("INSERT INTO walllikes_track (comment_id,member_id) VALUES('".$entry_id."','".$member_id."')");
	
	$result = mysql_query("SELECT clikes FROM wallcomments where c_id = ".$entry_id." ");
...
{
	$result = mysql_query("update wallcomments set clikes=clikes-1 where c_id= ".$entry_id);
	$result = mysql_query("delete from walllikes_track where comment_id=".$entry_id." and member_id=".$member_id);
...
{
	$result = mysql_query("SELECT wallusers.*,wallusers.mem_id as uid,walllikes_track.* FROM wallusers, walllikes_track where walllikes_track.comment_id = ".$entry_id." and wallusers.mem_id = walllikes_track.member_id");
