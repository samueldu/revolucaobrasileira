...
$user_idx = $Wall->getuserid($Gid);
$user_id = $user_idx['customer_id'];
$facebook_id['facebook_id'] = $user_idx['facebook_id'];
...
if(!$logged_id)
    ...
    //print $logged_id." ".$Gid." ".$facebook_id['facebook_id'];
    

$member_avatar = $Wall->getAvatar($logged_id,$facebook_id);
...
elseif( $ActionMore) // more posting paging
{
	...
	$posted_on = isset($_REQUEST['p']) ? $_REQUEST['p'] : "";
	$_GET['id_wall'] = isset($_REQUEST['x']) ? intval($_REQUEST['x']) : "";  
...
{
	$show_more_button = 1;
	$result = $Wall->GetMyPosts( $user_id, 0,$_GET['origem'],$_GET['id_wall'] );  	
}
...
while ($row = @mysql_fetch_array($result))
{
	...
	<div class="friends_area" id="record-<?php  echo $row['p_id']?>">
		...
		{
			$post_avatar = $member_avatar;
            
            $sql = "SELECT * from customer where customer_id=".$row['posted_by']." limit 1"; 
            $username_gets = mysql_query($sql);
            while ($names = mysql_fetch_array($username_gets))
            {
                $row['firstname'] = $names['firstname'];
                $row['lastname'] = $names['lastname'];
                $row['facebook_id'] = $names['facebook_id'];
                $row['twitter_id'] = $names['twitter_id'];
                
            }
            
            $post_avatar = $Wall->getAvatar($logged_id,$row);
           
            ?>
            
			
			<a href="<?php echo $wallUrl.'?id_wall='.$row['userid']."&origem=4&id=".$user_id;?>" target="_parent">
		...
		//print $row['posted_by'];
			...
			while ($names = mysql_fetch_array($username_gets))
			{
				...
				$username_2 = $names['customer_id'];
				$user_id_2 = $names['customer_id'];
				$fname_2 = utf8_encode($names['firstname'].' '.$names['lastname']);
			}
			
			$s_post_avatar = $Wall->getAvatar($row['posted_by'],$rowTrem);
			?>
			<a href="<?php echo $wallUrl.'?id_wall='.$username_2."&origem=4&id=".$user_id;?>" target="_parent">
				<img src="<?php echo $s_post_avatar;?>" style="float:left; padding-right:9px;" width="50" height="50" border="0" alt="" />
			</a>
			<label style="float:left;" class="name">
			   <a href="<?php echo $wallUrl.'?id_wall='.$username_2."&origem=4&id=".$user_id;?>" class="azul nome_comentario" target="_parent">
				<?php echo $fname_2; ?>
			   </a> 
                 <?if(isset($_GET['origem']) and !isset($row['origem']))
                   $row['origem'] = $_GET['origem'];
                   print $Wall->getOrigemName($row['userid'],$row['origem']);
                   ?>
		 ...
		 <?php
		   ...
		   <?php
			...
			elseif($days == 0 && $hours == 0 && $minutes == 0)
			 ...
			 <?php if($logged_id == 0){?>
			 <a href="javascript: void(0)" class="azul" onclick="login()" ><?=$txt_comments?></a> -
			 <?php }else {?>
			 <a href="javascript: void(0)" id="post_id<?php  echo $row['p_id']?>" class="showCommentBox azul">Comentar</a> 
			 <?php }?>
		...
		<div id="CommentPosted<?php  echo $row['p_id']?>">
			...
			if($comment_num_row > 0)
			{
				...
				foreach($allrows as $rows)
				{
					...
					$comm_avatar = $Wall->getAvatar($rows['userid'],$rows);
