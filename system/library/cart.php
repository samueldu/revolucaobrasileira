...
final class Cart {
		...
		foreach ($this->session->data['cart'] as $key => $value) {
			...
			if ($product_query->num_rows) {
				...
				$product_data[$key] = array(
					...
					'estoque'      => $product_query->row['quantity']					
				);
			} else {
				$this->remove($key);
			}
	...
	public function celula($dados)
	{
		...
		$estoque = 0;
		  ...
		  if(isset($dados['options'][0]['option_value']))
		  {
				
				$variacao = 1;
				
			  foreach($dados['options'][0]['option_value'] as $key=>$value)
			  {
					$estoque = $estoque + $dados['options'][0]['option_value'][$key]['quantity'];
			  }
		  }
		  else
		  {
				  $estoque = $dados['quantity'];  
		  }  
		  
		  //print $dados['model'].$estoque.'<BR>';
		  
		  if(preg_match('|Chrome/([0-9\.]+)|',$_SERVER['HTTP_USER_AGENT'],$matched)) {
			$mostra = 0;
		  }
		  else
		  {
		  ...
		  if($estoque == 0)
		  {
				if(TEMPLATE == "lojavilla" or TEMPLATE == "armazem")
				{
					if($mostra == 1)
						$indisponivelImg = "<div style=\"position:relative\"><img style=\"position:absolute; \" src=\"catalog/view/theme/".TEMPLATE."/image/esgotado.png\" alt=\"Produto indisponível\" /></a>";
					elseif($mostra == 0)
						$indisponivelTexto = "<span style='color:#993333'>Produto Esgotado</span>"; 
				}
				else
				{
				if($mostra == 0)   
		   ...
		   $out = "<a href=\"".$dados['href']."\">
		  $indisponivelImg
		  <img src=\"".$dados['thumb']."\" title=\"".$dados['name']."\" alt=\"".$dados['name']."\" /></a>
		  <div>".$dados['name']."</div>
		  <div>";
		  if ($dados['rating']) { 
		  $out .="<img src=catalog/view/theme/".TEMPLATE."/image/stars_".$dados['rating'].".png alt=\"".$dados['stars']."\" />";
		   } 
		  $out .= "</div>
		  ...
		  $out .= $indisponivelTexto;
		   
		  
		   if ($display_price == 1) { 
		   if (!$dados['special']) { 

		   $out .= "<div><span class=precoProduto><a href=".$dados['href']." class='precoProduto'>".$dados['price']."</a></span></div>";
		  
		  
			if(isset($percentual))
			$out .= "<div class=\"desconto\">".$percentual."% OFF</div>";
			
		  
		   } else { 
		  
		  $out .= "<div style=\"color:#858585\"><a href=".$dados['href']." class='precoProduto'><span class=apenas>De</span> <span class='precoDe'> ".$dados['price']." </a></span></div> 
		  <div class='apenas'><a href=".$dados['href']." class='apenas' rel=\"nofollow\">Por <span  class=precoProduto>".$dados['special']."</span></a></div>";
		  
		   } 
		   
		  if(isset($dados['special']) and $dados['special'] != ""){
			$priceCalculo = $dados['special'];
			
			$percentual = number_format(100-((str_replace(",",".",str_replace(".","",(str_replace("R$ ","",$dados['special']))))) * 100) / str_replace(",",".",str_replace(".","",(str_replace("R$ ","",$dados['price'])))),0,",",".");
			
		  }
		  else
		  {
			$priceCalculo = $dados['price'];
		  }

		  $priceValue = str_replace(",",".",str_replace(".","",(str_replace("R$ ","",$priceCalculo))));
		  
		 // print $dados['price']."<BR>";

			  for($i = 1; $i<=$this->config->get('config_n_parcelas'); $i++){
			  $valorParcela = ($priceValue/$i);
			  if($valorParcela >= $this->config->get('config_valor_parcelamento'))
				$parcela[$i] = $valorParcela;
				}
 
		  
				 
			$out .= "<div class=\"parcelas\"><a href=".$dados['href']." class='parcelas'>";
			
			if(isset($percentual))
			$out .= "<div class=\"desconto\">".$percentual."% OFF</div>";
			
			if(isset($parcela))
			$out .= "".count($parcela)."x de R$ ".number_format($priceValue/count($parcela), 2, ',', ' ')." sem juros</a></div>";
		   
		  if($variacao == 0)
		  { 
		  $out .= "
		  <a id=\"details ".$dados['product_id']."\" rel=\"nofollow\" href=\"javascript:quickView( ".$dados['product_id'].",$('#details ".$dados['product_id']."'));javascript:dimOn();\" style=\"text-transform:uppercase;color:#858585;font-size:10px\"  class=\"button_add_small\">&nbsp;</a>";
		  }
		   }
