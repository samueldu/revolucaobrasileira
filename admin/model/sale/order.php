...
class ModelSaleOrder extends Model {
	...
	public function updateShippingAddress($order_id, $data) {
		$this->db->query("UPDATE `" . DB_PREFIX . "order` SET shipping_firstname = '" . $this->db->escape($data['shipping_firstname']) . "', shipping_lastname = '" . $this->db->escape($data['shipping_lastname']) . "', shipping_company = '" . $this->db->escape($data['shipping_company']) . "', shipping_address_1 = '" . $this->db->escape($data['shipping_address_1']) . "', 
		shipping_address_2 = '" . $this->db->escape($data['shipping_address_2']) . "', 
		shipping_address_3 = '" . $this->db->escape($data['shipping_address_3']) . "',  
		shipping_bairro = '" . $this->db->escape($data['shipping_bairro']) . "', 
    ...
    $this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . (int)$order_status_id . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
		...
		if($data['order_status_id'] == "7")
		{
			...
			WHERE order_status_id = '7' AND order_id = '" . (int)$order_id . "'";
					...
					foreach($product_query->rows as $product)
					{
						...
						if ($product['subtract'])
						{
							...
							foreach ($option_query->rows as $option) 
							{
								...
								$sql = "UPDATE " . DB_PREFIX . "product_option_value SET quantity = (quantity + " . (int)$product['quantity'] . ") WHERE product_option_value_id = '" . (int)$option['id'] . "'";
		...
		*/
      	...
      	if ($data['notify']) {
			...
			if ($order_query->num_rows) {
				...
				$message .= '<b>'.$order_query->row['status'] . "</b><BR><BR>\n\n";
				$message .= $language->get('text_invoice') . "<BR>\n";
				$message .= html_entity_decode($order_query->row['store_url'] . 'account/invoice?order_id=' . $order_id, ENT_QUOTES, 'UTF-8') . "<BR><BR>\n\n";

				if ($data['comment']) {
					$message .= $language->get('text_comment') . "<BR>\n\n";
					$message .= strip_tags(html_entity_decode($data['comment'], ENT_QUOTES, 'UTF-8')) . "<BR><BR>\n\n";
				}
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			$color_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = '" . (int)$order_query->row['order_status_id'] . "'");

			if ($color_query->num_rows) {
				$status_color = $color_query->row['color'];
			} else {
				$status_color = '#000000';
			}
			...
			$order_data = array(
				...
				'shipping_bairro'         => $order_query->row['shipping_bairro'],
				...
				'rastreamento'              => $order_query->row['rastreamento'], 
			);
	...
	public function getOrders($data = array()) {
		...
		SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS name, (SELECT os.name, os.color FROM " . DB_PREFIX . "order_status os 
		WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, 
		o.date_added, o.total, o.currency, o.value FROM `" . DB_PREFIX . "order` o";
		
		$sql = " 
		SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS name, o.date_added, o.total, o.currency, o.value , `status`.name as status, `status`.color
		FROM `order` o,
		(SELECT os.name, os.color, os.order_status_id, os.language_id FROM order_status os) AS status
		WHERE o.order_status_id = status.order_status_id ";

		if (isset($data['filter_order_status_id']) && !is_null($data['filter_order_status_id'])) {
			$sql .= "and status.order_status_id = '" . (int)$data['filter_order_status_id'] . "'";
		} else {
			$sql .= "and status.order_status_id > '0'";
		}
	...
	public function salvarRastreamento($order_id,$rastreamento) {
	
		$query = $this->db->query("UPDATE `" . DB_PREFIX . "order` SET rastreamento = '" . $rastreamento . "' WHERE order_id = '" . (int)$order_id . "'");
	}
    ...
    public function riskHistory($order_id,$partner) {
    	
    	if($partner == "T-Clear-Sale")
    	{        
	        $sql = "SELECT * FROM " . DB_PREFIX . "clearSale
	        inner join clearSaleTgDetails on clearSaleTgDetails.status = clearSale.status
    		WHERE clearSale.`idPedido` = '" . (int)$order_id . "'";
	    }
