...
class ModelCatalogProduct extends Model {
	...
	public function editProduct($product_id, $data) {
		...
		foreach ($data['product_description'] as $language_id => $value) {
	     	...
	     	$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'product_id=" . (int)$product_id . "', keyword = '".$this->retornaUrl($value['name'],(int)$product_id)."'");
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', meta_keywords = '" . $this->db->escape($value['meta_keywords']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', description = '" . $this->db->escape($value['description']) . "'");
		}
	...
	public function emailAvise($product_id,$data)
	{
		                          
		$email = array();
		
		$query = $this->db->query("SELECT product_id, email FROM " . DB_PREFIX . "product_avise WHERE product_id = '" . $product_id . "' and enviado = '0'");
		...
		$query = $this->db->query("UPDATE " . DB_PREFIX . "product_avise set enviado = '1' WHERE product_id = '" . $product_id . "'");
	...
	public function addFeatured($data) {
		...
		if (isset($data['product_featured']) and is_array($data['product_featured'])) {
	...
	public function retornaUrl($nome,$product_id)
	{
		
    $nome = str_replace("%","",$nome);   
	$nome = str_replace("&amp;","",$nome);    
	
	$nome = $this->addFeatured($nome); 

	$nome = str_replace("/","",$nome);    
	$nome = str_replace(" ","-",$nome); 
	$nome = str_replace("\"","",$nome);       
	$nome = str_replace("--","-",$nome);    
	$nome = strtolower($nome).'-'.$product_id;   
	...
	{
		$a = array('À', 'Á', 'Â', 'Ã', 'Ä', 'Å', 'Æ', 'Ç', 'È', 'É', 'Ê', 'Ë', 'Ì', 'Í', 'Î', 'Ï', 'Ğ', 'Ñ', 'Ò', 'Ó', 'Ô', 'Õ', 'Ö', 'Ø', 'Ù', 'Ú', 'Û', 'Ü', 'İ', 'ß', 'à', 'á', 'â', 'ã', 'ä', 'å', 'æ', 'ç', 'è', 'é', 'ê', 'ë', 'ì', 'í', 'î', 'ï', 'ñ', 'ò', 'ó', 'ô', 'õ', 'ö', 'ø', 'ù', 'ú', 'û', 'ü', 'ı', 'ÿ');
