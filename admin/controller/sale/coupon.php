...
class ControllerSaleCoupon extends Controller {
	private $error = array();
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            if(isset($this->request->post['emails']))
            {
			    ...
			    foreach($entry_array as $key=>$value)
			    {
				    ...
				    $message = "Oi o seu cupom de desconto : ".$this->request->post['code'];
				    $mail = new Mail();
				    ...
				    $mail->setHtml(html_entity_decode($message, ENT_QUOTES, 'ISO-8859-1'));
				    $mail->send();
			    }
            }
  	...
  	private function getForm() {
		...
		if(!isset($this->data['coupon_description'][2]['name']))
		$this->data['coupon_description'][2]['name'] = "Cupom de desconto";
		
		if(!isset($this->data['coupon_description'][2]['description']))
		$this->data['coupon_description'][2]['description'] = "Cupom";
