...
class ControllerSaleOrder extends Controller {
	private $error = array();
  	...
  	public function getForm() {
		...
		if ($order_info) {
            ...
            $this->data['entry_risk_partner'] = $this->language->get('entry_risk_partner');
			...
			$this->data['tab_risk'] = $this->language->get('tab_risk'); 
			$this->data['risk_history'] = array();
			/* analise de risco */
			if($this->config->get('config_risk_status') == 1)
			{
				$name = $this->config->get('config_risk_partner');
				$this->data['risk_partner']  = $name;
				$this->data['risk_history'] = $this->model_sale_order->riskHistory($order_id,$name); 
			}
			
			$this->data['entry_rastreamento'] = $this->language->get('entry_rastreamento');
			...
			if ($order_info['rastreamento']) {
				$this->data['rastreamento'] = $order_info['rastreamento'];
			} else {
				$this->data['rastreamento'] = '';
			}
			...
			if($this->data['payment_method'] == "Ipagare" or $this->data['payment_method'] == "Ipagare web")
      		{
      			    $resultx['payment_method'] = $this->model_sale_order->getIpagare($order_info['order_id']);
      			    
        			$this->data['payment_method'] = $resultx['payment_method'];
      		}
			...
			$this->data['shipping_bairro'] = $order_info['shipping_bairro'];
	...
	public function generate() {
  	...
  	public function salvarRastreamento() {
  		
  		$order_query = $this->db->query("SELECT *, os.name AS status FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_status os ON (o.order_status_id = os.order_status_id AND os.language_id = o.language_id) LEFT JOIN " . DB_PREFIX . "language l ON (o.language_id = l.language_id) WHERE o.order_id = '" .$this->request->post['order_id'] . "'");
         		
		$this->load->model('sale/order');
		
		$this->model_sale_order->salvarRastreamento($this->request->post['order_id'],$this->request->post['rastreamento']);
		
		$this->load->language('mail/rastreamento');

		$subject = $this->language->get('text_subject');
		
		$subject = str_replace("%s", $this->request->post['order_id'], $this->language->get('text_subject'));
		
		$value = $this->request->post['email'];
		
		$message = $this->language->get('text_order'). " ".$this->request->post['rastreamento']; 
		$message .= "<BR><BR>".$this->language->get('text_invoice') . "<BR>\n";
		$message .= html_entity_decode($order_query->row['store_url'] . 'account/invoice&order_id=' . $this->request->post['order_id'], ENT_QUOTES, 'UTF-8') . "<BR><BR>\n\n";
	...
	public function history() {
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			$return = $this->model_sale_order->addOrderHistory($this->request->get['order_id'], $this->request->post);
