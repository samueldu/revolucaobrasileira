...
class ControllerCatalogProduct extends Controller {
	private $error = array(); 
  	...
  	public function update() {
		...
		//	exit;
			...
			$this->emailAvise($this->request->get['product_id'], $this->request->post);
	...
	public function emailAvise($id,$data)
	{
		
		$this->load->language('mail/aviseme');
		
		$this->load->model('tool/image');   
		
		$emails = $this->model_catalog_product->emailAvise($id, $data);
		
		$produto = $this->model_catalog_product->getProduct($id);
		
		if ($produto['image'] && file_exists(DIR_IMAGE . $produto['image'])) {
				$image = $this->model_tool_image->resize($produto['image'], 160, 160);
			} else {
				$image = $this->model_tool_image->resize('no_image.jpg', 160, 160);
			}
		
		$link = '<a href='.$this->config->get('config_url').'index.php?route=product/product&product_id='.$id.'>';
		
		$subject = sprintf($this->language->get('text_subject'), $produto['name'],$this->config->get('config_name'));
		
		$message = $link.'<img src='.$image.' border=0> Clique aqui e confira</a>';
